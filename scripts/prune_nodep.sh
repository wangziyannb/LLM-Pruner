...
prune0='llama_prune_0.2_without_tune_block_param_first_4_30_0.25_nodep_shape_index_grouping_V'
#prune1='llama_prune_0.2_without_tune_block_param_first_0_32_0.2_nodep_shape_index_grouping'
#prune2='llama_prune_0.2_without_tune_block_param_first_0_32_0.2_nodep_shape_index_grouping'
#prune3='llama_prune_0.2_without_tune_block_vectorize_0_32_0.2_nodep_shape_index_grouping'

task_skip(){
  CUDA_VISIBLE_DEVICES=$2 python prune.py --base_model "$4" --pruning_ratio 0.25 --device cpu --eval_device cuda --mask_type_mha v --mask_type_mlp gate_proj --head_dim --block_wise --block_mlp_layer_start 4 --block_mlp_layer_end 30 --block_attention_layer_start 4 --block_attention_layer_end 30 --save_ckpt_log_name "$1" --pruner_type taylor --taylor "$3" --save_model
}
task_noskip(){
  CUDA_VISIBLE_DEVICES=$2 python prune.py --base_model "$4" --pruning_ratio 0.2 --device cpu --eval_device cuda --head_dim --block_wise --block_mlp_layer_start 0 --block_mlp_layer_end 32 --block_attention_layer_start 0 --block_attention_layer_end 32 --save_ckpt_log_name "$1" --pruner_type taylor --taylor "$3" --save_model
}
task_noskip_origin(){
  CUDA_VISIBLE_DEVICES=$2 python hf_prune.py --pruning_ratio 0.2 --device cpu --eval_device cuda --block_wise --block_mlp_layer_start 0 --block_mlp_layer_end 32 --block_attention_layer_start 0 --block_attention_layer_end 32 --save_ckpt_log_name "$1" --pruner_type taylor --taylor "$3" --save_model
}
task_post_training(){
...
task_skip "$prune0" 3 'param_first' $BASE_MODEL & \
#task_noskip "$prune1" 2 'param_second' $BASE_MODEL & \
#task_noskip "$prune2" 3 'param_mix' $BASE_MODEL & \
